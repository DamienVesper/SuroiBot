...
class Seek extends Command {
    ...
    run = async (interaction: ChatInputCommandInteraction): Promise<void> => {
        ...
        const seekPos = interaction.options.getInteger(`time`, true) * 1e3;
        ...
        if (seekPos > player.queue.current.duration!) {
            await interaction.followUp({ embeds: [this.client.createDenyEmbed(interaction.user, `The seeked position is greater than the song duration!`)] });
            return;
        }
